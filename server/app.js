...
app.get('/api/tickets/', async (req, res) => {
  ...
  if (req.query.searchText) {
    const filterText = tickets.filter((text) => text.title.toLowerCase().includes(req.query.searchText.toString()));
    res.send(filterText);
  } else if (req.query.ticketId) {
    const filterText = tickets.filter((text) => text.id.toString().toLowerCase() === (req.query.ticketId.toString().toLowerCase()));
    res.send(filterText);
  } else {
    res.send(tickets);
  }
